...
Open the printed URL. The game opens on the main menu, over a stage a bot is driving under a drone camera: **Campaign** works through Taiga's four stages, each unlocked by the one before it; **Time trial** re-runs the ones you have opened, for the clock; **Roam** picks any seed at all and shows you the map it builds — spinning, with the route drawn on it — before you commit; **Options** covers the HUD, the renderer and the controls. Pick a stage and you drop into a 3-2-1 countdown. Keyboard (all rebindable in Options): arrows/WASD drive, Space is the handbrake (unsticks the rear), Left Shift burns the finite booster, Q/E shift the manual car, C returns to the main menu, V cycles the camera (chase/hood), B resets a lost car back to the track, R restarts the stage.
