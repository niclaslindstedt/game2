...
// WHAT STANDS IN ONE CHUNK OF THE WORLD, built once and never touched
// again: the trunks and boulders of the forest, the fallen timber, the
// buildings and their yards, the fences and the crowd, the bridges over
// the water, and the branches and public roads running off the stage.
// `world.ts` decides WHEN a chunk is built and when it is thrown away;
...
import { type Biome, type Community } from "./biome.ts";
...
export const PEBBLE_MAX = (SOLID_PROP_HEIGHT / 1.05) * 0.95;
export const PEBBLE_MIN = PEBBLE_MAX * 0.35;
...
export const BUILD_TILES = 3;
export const BUILD_SLICES = 1;
...
export function pace(dt: number): number {
  return Math.min(8, Math.max(1, Math.round(dt * 60)));
}

export type SceneryChunk = {
  group: THREE.Group;
...
export function buildPublicRoad(track: Track, road: SpurLine, beside: GroundBeside): THREE.Group {
  const group = new THREE.Group();
