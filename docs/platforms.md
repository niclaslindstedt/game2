...
- **ROAM**, which the released site does not offer. It is the one feature flag (`pwa/src/features.ts`) whose rule is not simply "a preview build": reading a map and picking a road off it is worth the screen it takes in a window somebody installed deliberately, with a keyboard, and it is the wrong first thing for a buyer to find on a phone — so the desktop app has it in every build and the store app never does. It is still a feature of the WEBSITE, offered in there by a rule rather than by code the shell adds; see [configuration.md](configuration.md).
