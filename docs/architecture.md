...
- `step(state, input)` advances one fixed 120 Hz timestep and returns the `GameEvent[]` that step emitted (`driftStart`, `driftEnd`, `takeoff`, `landing`, `splash`, `shift`, `respawn`, `finish`, …). The browser's render loop and the headless simulator call this same function — there is no other way to advance a run.
...
| Module          | Owns                                                                                                   |
| --------------- | ------------------------------------------------------------------------------------------------------ |
| `game/car.ts`   | The handling model: grounded + airborne steps, the drift state machine, gearboxes                      |
