...
| `EAS_PROJECT_ID`       | The Expo project the app builds under, until it is pinned in `native/app.config.js`; the `native` workflow reads it from a repository secret of the same name.                                                                  |
