...
- **...and short of it, the tail RUNS.** The wheel's own top is where a
  ...
  not: a flick held on the rear-driver at 100 km/h runs to a spin in about a
  second — backwards, if the road is wide enough — and a full lock held on
  the wheel alone still parks, because the wheel named that angle. This is
  the whole difference between a drift and a spin being something the
  player does, and why keeping a car on the road is a feat rather than a
  given.
