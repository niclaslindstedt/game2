...
- **THE AIR, once the wheels are not holding the car up** (`engine/game/aero.ts`). On the road the air is a rolling loss the surface owns; off the ground it is the only thing the car is touching, and it decides how fast a fall can possibly get and which way the car is pointing while it gets there. Drag is quadratic and acts on the WHOLE velocity — a car off a cliff is travelling mostly straight down, and a drag that only reached the forward speed let the fall accelerate without limit — so a fall has a terminal velocity, `v_t = sqrt(2·m·g / (rho·CdA))`. `CdA` is not one number: the box's three faces differ by a factor of nine (`TUNING.air.aero`'s `nose`, `plan`, `side`, scaled per car by `CarSpec.aero.slip`), blended by the SQUARE of each direction cosine — the cross-flow form, because a face a few degrees off the flow still has the air attached to it and is not yet the bluff plate it becomes once the flow separates. On the plain cosine a car flying five degrees nose-up is charged double its end-on drag, which is a jump that stops throwing the car. Nose-on the car is a dart and an ordinary jump barely feels the air; flat it is a sheet of plywood. Driving off a cliff at 90 km/h, the roster passes 180 km/h inside three seconds and settles between 238 and 285 km/h — past anything the road gives, which is the point.

- **...and the AIR POINTS THE NOSE, which is what a wing is for.** The same force does not act through the weight, and what is left over is a moment (`aeroTrim`). Two levers: the body's own plan area pushes through the middle of the car while `balance` puts the weight ahead of that, so a nose-heavy car noses over as it falls; and the WING (`CarSpec.aero.wing`, `wingArm`), much the longer of the two. One rule covers a jump and a fall at once, because a wing is two things to two flows — an aerofoil to the air running ALONG the car, where it makes downforce, pushes the tail down and lifts the nose (the saloon carries its nose 2.6° up over a jump taken at 110 km/h and 5.6° at 160, where the two cars with only a lip on the boot manage a third of a degree and two thirds), and a flat plate to the air coming UP through it in a fall, where it is pushed up at the same lever and puts the nose DOWN. Feathers on an arrow, and the same arithmetic. The trim is added to the arc the nose already follows and clamped by `attitude.pitchMax` like every other attitude in the game.

- **WHAT IS MEASURED AND WHAT IS A DIAL.** The areas, the coefficients and the density are quoted numbers and can be argued with from outside the game: the same sum with the real 9.81 in it rather than the arcade 1.6 g puts the roster at 158, 189 and 159 km/h falling flat, against the 160–190 the world quotes for a car dropped from a height, and `tests/aero_test.ts` holds every car inside 150–200 so a retune cannot quietly drift off it. Three knobs sit on top and are nobody's measurement: `air.gravity` (already 1.6 g, so a flight comes down briskly), `air.aero.bite` (the whole drag, and `v_t` goes as `1/sqrt(bite)` — half the bite is a fifth again more speed at the bottom of a cliff), and `air.aero.trim` (how far the air points the nose). Tune those; leave the measurements alone.

- **THE SPEEDO READS THE CAR'S SPEED THROUGH SPACE** (`travelSpeed`), vertical included — not its forward speed and not its ground speed. On the road the two are the same thing, since `vy` there is the road's own gradient under the wheels. Off it they are not: a car two seconds off a cliff has almost no horizontal speed left next to the vertical, and a needle counting only the horizontal part would fall toward zero through the fastest seconds of the run. Stated once in the engine so the cluster on the HUD and the dials on the dashboard cannot disagree.
...
So it is modelled as a SCHEDULE rather than a level: fronts crossing the biome at their own seeded times and strengths, and a run meets however many of them its length puts it in the way of. `sandAt(env, t)` is a pure function of the environment and the clock — two hashes and no state — so the same seed brings the same storms back on every replay, every rival's trace and every sim digest, and an endless run an hour in can be asked about its next front without anybody having kept a list. It exists only in a biome whose wind lifts the ground (`BiomeRules.blown`), which is the desert and nowhere else: a gale through a rooted forest is a gale.
...
- **The ground** — off the verge the car rides the terrain — and it rides
  ...
  and 200 km/h depending on the land — which is the limit worth having,
...
- **The ends of the stage** — off the road is not only sideways. The stage
  is drawn and shelved for one APRON (`startZone.apron`) past each end —
  the run-up before the start gate and the run-off past the flying finish —
  and that apron is road: flat at the gate's own height, with the terrain's
  shelf under it. Past it the stage is over and the terrain owns the ground,
  the same as if the car had driven off the side. Reverse away from the
  start line for long enough and the biome takes over, hills and all. A
  CIRCUIT has neither end (R22): the road behind its start line is the
  closing straight and the road past it is the opening one, each on a grade
  of its own, so no apron is drawn there and the terrain shelves none —
  a level slab over a lap that is falling away is a step in mid-road with
  nothing drawn on it to warn anybody it is there.
...
- **WHICH GROUND SUITS IT**, which is where the roster's identities now
  ...
  exactly nothing on gravel or tarmac. The frozen biome is its ground, and
