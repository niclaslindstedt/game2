...
2. **Corner-speed plan** — scans the curvature ahead; each corner caps speed at `√(a_lat/κ)`, distance-discounted by braking capability. `a_lat` is a FRACTION of what the car actually HOLDS (`latFraction × latHold`, off `engine/game/limits.ts` — the handling model's own numbers rather than a second guess at them), not a fixed number, so the same brain plans honestly in any car — and because the slide comes in relative to the same tyre, this one knob is what decides whether the bot ever drifts a corner or only ever flicks the handbrake at one. `latHold` is NOT the traction ceiling: the ceiling is the SCALE of the tyre's saturation curve, and what a car gets out of that curve is it read at the slip angle the car is really carrying (`heldSlip` — the wheel's own ask plus the room past it where the deepening forces balance the redirect). The difference is the whole drift model showing up in the plan, and it is a real difference: because `over` grows with speed, a car at rally pace holds MORE than its stated ceiling and a car pottering holds less, and a plan quoted off the flat ceiling is wrong in opposite directions at the two ends of the stage. The corner speed is solved rather than evaluated — the speed sets the lateral acceleration and the lateral acceleration sets the speed — by iterating from the tyre-limited answer, which lands within a tenth of a km/h in three passes. It is scaled by the grip the car will actually HAVE at that corner: the surface AHEAD times the rubber this car is on, exactly as `car.ts` reads it. Without that the plan is blind to the whole surface half of a car's character, and no tire in the catalog ever shows up as pace. Hard corners are planned HOT — rally style, the drift scrubs the excess — by `hotEntry` m/s scaled by how much overspeed this car's slide will actually SCRUB, which is the only thing a hot entry spends: the angle the car parks a held slide at (`heldSlip`) read through the same squared sine the tyres burn speed off with, against the profile's `rotationRef` (a reference slip angle). A front-driver has plenty of rotational authority and almost no angle to spend it at, and a driver who read the authority instead would carry a rear-driver's entry speed into a car that answers by washing straight on: the slide is what brings the nose round in there, so a car that goes sideways can carry more in than one that pushes. And then it reads what is BESIDE that corner (`exposure`): a driver plans off the grip he expects to have AND off what happens if he does not get it, and those are different questions — the same bend is taken one way with a field on the outside of it and another with a lake. Where running wide would end the run rather than cost a second, the plan gives up a share of the corner speed and asks for proportionally less of the hot entry. Before it did, a stage laid along the country (R34) put water on the outside of a fast corner and the quickest crews drove into it until the clock ran out — three hundred respawns in a single run. It is read from the terrain field a car's-width off the road, cached per sample, and only at the corners the scan actually reaches, because a terrain query costs far more than anything else in this loop. The plan also knows what a JUMP does to braking: past the nearest lip in the horizon the car is in the air and then skittering on its landing, neither of which slows it, so a corner beyond a lip is planned over the road up to the lip plus what is left past the glide (`LIP_GLIDE` seconds of travel) — without it a car lands at full pace a few lengths from a hairpin with nothing left but the slide, which at that speed is a spin.
