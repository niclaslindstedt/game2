...
One more number, and it is not a skill or a temperament: `gridPhase`, where in its own blip a car starts. It comes off the **start number** rather than the crew (golden-ratio spaced, so however deep the entry list is cut the field stays spread across the cycle), because two identical drivers stood side by side should still not blip in unison. Fourteen crews on one clock is one loud engine; fourteen a fifth of a blip apart is a start line.
