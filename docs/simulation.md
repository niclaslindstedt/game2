...
- `watchField(field, ticks, limit)` — a frame's worth of ticks, at race speed. This is what runs behind the results card: once the player's roll-out ends, the card's backdrop becomes the run-out itself, from the drone, on the leader of what is left, and SPECTATE (`pwa/src/game/spectate.ts`) drops the camera onto that car with a strip and two buttons that walk the field.
