...
**It does not think on every step.** The physics runs at `TUNING.physicsHz` because that is what integrates a car; the driver re-reads the road at `TUNING.botHz` (half that), and between decisions the hands stay where they were put — which is also what hands do. Everything in the list below is one decision, and the corner scan that dominates it therefore runs half as often. The hold lives in `bot.ts` keyed on the run, so no caller can forget it, and the decision step is read off the run's own clock, so a replay, a headless sim and the live game all decide on the same steps. Counter-intuitively a slower driver drifts MORE, not less: holding the lock longer makes slides deeper and longer, and 60/30 lands closer to the drift figures the game was tuned at (120 Hz, deciding every step) than 60/60 does.
