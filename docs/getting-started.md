...
- **Campaign** — a location (Taiga, for now) and its six stages, each one unlocked by getting on the **podium** of the one before it, and the location after it unlocked by winning its **championship**. Locked stages wear a grey border and a padlock; open ones wear green. The ladder climbs in both length and difficulty, from a 1-minute forest road to seven minutes of hairpins and jumps in a night storm — and then changes discipline: the last two are **circuits**, three laps of a closed lap that comes back to its own start line, where you get to learn the road and the clock is the whole opponent.
  ...
  And every stage is worth **championship points**: three for the win, two for second, one for third, nothing below that — to you and to the fourteen crews alike. At the line the card says what the place paid you and where the season now stands, and **FULL RESULTS** opens the whole sheet: all fifteen crews in finishing order, their times, what the stage paid each of them, and what they have for the location. A stage run again keeps whichever attempt paid you MORE, so a lap driven for fun can never cost you a title.

  Those points add up over a location, and they are the lock on the next one: the **CHAMPIONSHIP** panel under the stage list shows where you stand, **CONTINUE** takes you to the next stage to drive (forward while there are stages you have not seen, then back to the first one you have not WON), **STANDINGS** shows the whole table, and **RESET SEASON** tears it up and starts again — the stages stay cleared; only the points go. To leave a country you have to drive every stage of it and finish **top of its table**, which in practice means winning stages rather than scraping onto podiums.
