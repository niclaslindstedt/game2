...
import { offParallel, strands } from "./roads-strands.ts";
import { analyzePlaces } from "./roads-places.ts";
import {
  metricScore,
  rate,
  under,
...
export function analyzeRoads(track: Track, terrain: TerrainField): MetricReport {
  ...
  const places = analyzePlaces(track, terrain);
  findings.push(...places.findings);
  const {
    orphanFlips,
    lots,
    badLots,
