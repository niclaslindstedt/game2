...
// R40 — THE BIOME a stage is built in, as data.
//
...
// whether the land holds water, whether the wind has piled it into
...
// read the land when it picks which way a corner turns, and whether a
...
// and the tooling all build the same landscape without being told twice.
...
/** One kind of biome. `forest` scales the trunk density across the whole
...
/** Meters of region-noise period — how big one kind of biome is. Several
...
/** How the bare rock of a land stands, and what lies on it. */
export type BiomeLand = {
  /** Multiplier on the whole of the relief the `elevation` dial asks for.
   * Under 1 is a biome that has been worn flatter than the taiga's. */
  relief: number;
   ...
   * where the biome has none. `amp` is how high a ridge stands over the
   ...
   *
   * The row is the biome at the DUNE dial's rest position, and the dial
   ...
   * for. Never null on a row a dial can move: a biome whose sand has
   ...
   * biome has no water its hollows do not fill — they flatten into pans,
   * and this is the level they flatten to. It also keeps the whole of a dry
   * biome above the table, so nothing downstream mistakes a low plain
   * for a lake bed. In a mountain biome it is the VALLEY FLOOR: the flat
   * a glacier left, with the lakes cut into it and the villages on it. */
  floor: number | null;
  /** THE MASSIF (R47): a mountain range the whole biome is made of, or
   * null where the biome has only the taiga's low chains. `scale` is the
   ...
   * goes under snow on anything but a face, or null in a biome that has
  ...
  /** R47 — how hard the route search READS THE BIOME when it chooses
   * which way a corner turns, 0 (the dice) to 1 (always the side the
   * road can follow). A road in a mountain biome is laid along its
   ...
   * land stands over it, and the search accepts what it would
   * otherwise refuse. */
  tunnels: boolean;
  /** Multiplier on the grade a road may follow the land at
  ...
  grade: number;
  /** Multiplier on how far the road LAGS the biome it is laid along
   ...
   * biome it has been allowed to fall. A forest road is surveyed, cut
  ...
  lag: number;
  /** Multiplier on how far a road may stand OFF the biome (R34's
...
export type BiomeRules = {
  ...
  label: string;
  /** The kinds of biome the stage is quilted from, coarsest scale. */
   ...
   * water table whatever the quilt rolled, or null in a biome whose
  ...
  timber: string | null;
  /** What an unsealed road in this biome is bladed out of: graded stone,
   * or the sand the biome is made of. The physics tells the two apart
   * (`TUNING.surfaces`); everything about the road's SHAPE treats them the
   * same (`isLoose`). */
  loose: "gravel" | "sand";
  /** Whether the biome carries WATER at all: a groundwater table that
   * surfaces, basins that fill, fords and bridges on the route, and the
   * watercourses R18 traces through them. A dry biome has none of it. */
  water: boolean;
  /** Whether the forest sheds DEAD WOOD — blowdowns down the fall line,
   * root plates, cut stumps. A biome with no trees to fall has none. */
  deadwood: boolean;
  land: BiomeLand;
  /** The weathers the sky over this biome can be in. */
  ...
  rain: boolean;
  /** Whether THE WIND PICKS THIS BIOME UP AND CARRIES IT: whether a
   ...
   *
   * It is the flag the SANDSTORM row is offered on, so a fourth biome
  ...
  latitude: number;
  /** R37/R39 — whether anybody LIVES in this biome: the homesteads off
   ...
   * of them — so a desert stage is empty biome, and the tarmac it meets
   ...
   * outside the barn. Only where the land is farmed at all. */
  farms: boolean;
  /** R40 — what kind of HOUSE stands on its yards and along its streets:
   * the Nordic timber house, or the alpine chalet (`HouseStyle`). A biome
   * nobody lives in still names one, so the type has no hole in it. */
  houses: "nordic" | "chalet";
  /** R41 — whether the biome carries a RAILWAY: a single track laid across
   * the map before the rally, that the route may cross square on a ramp and
   * that a train runs down every so often. */
  railway: boolean;
  /** R43 — whether the biome MAKES POWER: the wind farms on its high
   * ground and the solar farms on its flats. A modern, settled biome
...
/** The five kinds of biome a taiga stage is quilted from. Lakeside and
 ...
 * in every hollow. The biome every rule was written against, so every
...
export const TAIGA: BiomeRules = {
  ...
  land: {
    ...
    // a biome that never has one.
    //
    // A BAND IS A PERCENTILE OF ITS OWN BIOME, not a height that can be
    ...
    // tenth of the biome and goes solid only on the very tops. The
    ...
    // high biome came out grey end to end with no soil and half its
    ...
    // which is what these are, the biome is green and its tops are bare.
 ...
 * biome has instead is the sand the wind has piled across it. */
export const DESERT: BiomeRules = {
  ...
  land: {
    relief: 0.7,
    mountains: 0.45,
    // THE SAND IS THE BIOME. A dune every 300 m across the wind,
    ...
    // be to this biome what the hills are to that one: the layer a
    ...
    // row is the biome at its rest position and nothing more.
    dunes: { amp: 22, scale: 300, stretch: 4, field: 1500 },
    // Fourteen metres over the lake table. The ROAD rides its own rolling
    // profile on top of the biome (`elevation.amplitude`, up to ten
  ...
  rain: false,
  // The whole biome is loose and dry, so the storm here is made of the
  // biome: a haboob, and the one weather in this game that arrives.
  blown: true,
  latitude: 33,
  ...
  // solar farm for: the desert is not a place for the modern biome's
  // machinery.
  energy: false,
};
...
/** The five kinds of mountain biome, and they are ZONES more than they
...
/** The high mountains: a massif of ridges and valleys the whole biome is
...
export const ALPINE: BiomeRules = {
  ...
  land: {
    ...
    // ABSOLUTE lines and subtracts the biome's own base off them
    // (`landOf`), so a biome standing above its snowline gets a negative
    // one and is white from its valley floor up, which is what a biome
    ...
    // when it fits the biome better by `massif.contour.margin`, so the
    ...
    // the biome on its walls and in its cuts.
    grade: 1.2,
    // A pass is SURVEYED — cut into the flank, carried on walls, held on a
    // long steady line the whole way up. It reads the biome further ahead
 ...
 * biome every seed was built in before there was a choice. */
