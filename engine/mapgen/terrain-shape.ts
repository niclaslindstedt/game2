...
// road's earthworks raised or cut the biome, and what ceiling does R31's
...
export function createShaping(
   ...
   * the cutting the road imposes, bounded by the biome and eased back
   * onto it.
   *
   ...
   * lands on it. So the bench is bounded BY the biome on whichever side
  ...
  const shelfBeyond = (index: number, d: number, lateral: number, lip: number, far: number) => {
    ...
    // R34 — a cut is only a cut where there is biome to cut INTO. On a
    ...
    // the biome — at LEAST that slope, not only where the dice drew a
    ...
    // samples.
    // ...and the other way round on the side the land stands OVER the
    // road: a side the noise drew as falling is no embankment there — a
    // fall bounded by the far field is the land itself from the lip,
    ...
    if (far < s.elevation) {
      ...
      // (`letGo`), never eased toward the land from the lip.
    ...
    // biome, and is eased up onto it from there. The cone is the ceiling
  ...
  /** The FILL the sample at `index` stands out over the land by, seen
   * from `d` metres off, eased back onto it the way `shelfBeyond` eases an
   * embankment — and `far` where the road stands at or under the land.
  ...
  /** The CUT the sample at `index` takes out of the land, seen from `d`
   * metres off — its bench, level at the road, eased back up onto the
   * biome the way `shelfBeyond` eases a cutting's side — and `far`
   * where the road stands at or over the land. Level rather than at the
   ...
   * and a fold where it does not is the biome's own. */
  const shapeAt = (x: number, z: number): void => {
    ...
    // relief render shows up as a hairline running across the land.
    if (!near || near.d > CORRIDOR_RANGE) {
      ...
      // left the land over the bore untouched, and the lattice cell
      ...
      // dozen metres over the land, was simply dropped for the lower
      ...
      // FILL — what stands over the land — because a cut is the cone's
      ...
      if ((near.other >= 0 || near.deep >= 0) && near.d >= lip) {
        ...
        // picked: what stands over the land is the larger of the two
    ...
    // bench climbing back onto the land past it are the CONE's business
    // below (R31 — the branch's own cone cuts the ground down to its shelf
    // and lets go toward the biome at a declared grade), and what it
    // stands OVER the land on is the fill carried here. The nearest
    ...
    // stands over the land here is the highest fill of every branch in
    ...
    // — `fillGrade`'s rule, read off the land under the branch) — never
    ...
    // right for the land between two arms of a stage — the lower one says
    ...
    // let go toward the bare land instead and every branch's embankment
    ...
    // not take, abandoned to the biome, and nobody blasts a cutting for a
    ...
    // biome LOWEST here, not the nearest: the cone is a min over every
