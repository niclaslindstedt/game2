...
export type Spur = {
  ...
  samples: SpurSample[];
  /** Full road width, meters — a country road, narrower than the stage's
   * own boulevard. */
  width: number;
...
/** Full width of a country road, meters — narrower than the stage's own
 * boulevard, which is part of how a borrowed road reads as somebody
 * else's. Exported because the junction apron has to be big enough to
 * swallow one. */
export const SPUR_WIDTH = 12;
...
/** Build the branch a junction leaves behind. `junction` is the stage
 * sample the tarmac starts or ends on; `away` is +1 when the branch
 * continues along the stage's direction of travel (the exit junction: the
 * tarmac carries on ahead while the route turns off) and -1 when it runs
 * back the way the tarmac came (the entry junction). Deterministic in the
 * seed and the junction's position. */
export function buildSpur(
  ...
  for (let s = 0; s <= length; s += SPUR.step) {
    if (s >= SPUR.length.min && escaped(x, z)) {
      length = s;
      break;
    }
    samples.push({ x, z, heading, elevation: y, s, surface: "asphalt", lift: 0 });
    if (s % SPUR.bend < SPUR.step) {
  ...
  // gone and what is left is a gravel lane heading out of the world.
  const sealedTo = length * SPUR.sealed;
  for (const sample of samples) {
    const sealed = sample.s < sealedTo;
    sample.surface = sealed ? "asphalt" : "gravel";
    const fade = Math.min(1, Math.max(0, (sealedTo - sample.s) / ROAD_CROSS.liftRamp));
    sample.lift = sealed ? ROAD_CROSS.asphaltLift * fade : 0;
  }
  return { atS, end, samples, width: SPUR.width, bounds: box };
}
