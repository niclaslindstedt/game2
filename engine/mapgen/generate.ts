...
function createPointField() {
  ...
  return {
     ...
     * closer than `minSelfDistance` to `c`? */
    blocked(c: Cursor): boolean {
      ...
      for (let dx = -1; dx <= 1; dx++) {
        for (let dz = -1; dz <= 1; dz++) {
          ...
          for (const p of bucket) {
            if (c.arc - p.arc < SELF_IGNORE_ARC) continue;
            const ddx = p.x - c.x;
            const ddz = p.z - c.z;
...
function inBounds(p: Cursor, worldBound: number): boolean {
  const bound = worldBound - BOUND_SLACK;
  return Math.abs(p.x) <= bound && Math.abs(p.z) <= bound;
}

type SameDirRun = { dir: 1 | -1 | 0; count: number; angle: number };
...
function recomputeSameDirRun(plans: SegmentPlan[], run: SameDirRun): void {
  run.dir = 0;
  run.count = 0;
  run.angle = 0;
...
function trackRun(run: SameDirRun, plan: SegmentPlan): void {
