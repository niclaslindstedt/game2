 ...
 * over the land, which is what the rig is (seed 0's land lies under
 ...
 * model's claim; which slope is the test's. The bank lands on the land
...
function onEmbankment(state: GameState): void {
  ...
  const biome = state.terrain.farHeightAt;
  ...
  state.terrain.groundAt = (x: number, z: number): number => {
    ...
    return Math.max(biome(x, z), bank);
  };
}
