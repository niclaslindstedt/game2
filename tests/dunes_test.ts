...
// a biome stripped to its rock to an erg, and what has to stay true across
// the whole of that travel.
//
// The one that costs the most to get wrong is the LAST: a dial only a sand
// biome reads must not move a biome with no sand in it. The taiga's
...
// that built it (the biome with the sand dialled off, subtracted from the
// same biome with it on, is the sand and nothing else). That is the only
...
/** The deepest sand anywhere in a square of desert biome at this dial
 * position, m — the biome with the dial where it is, minus the same
 * biome with the sand dialled away. */
 ...
 *
 * Off the SAND, the way `deepestSand` reads the depth: the biome with the
 * dial on, less the same biome with it off. Read off `surfaceAt` alone it
...
/** The same biome with the sand dialled away, kept: it does not depend on
...
describe("R40 — the dune dial", () => {
  it("prints the metres it builds, and builds the metres it prints", () => {
    ...
    // sand in the biome reaches it — not that the average does, which
    // would be a biome buried to its crests everywhere.
    for (const dial of [0.1, 0.22, 0.5, 1]) {
      ...
      // square of land holds a full-height erg is a fact about where
  ...
  it("hands the row itself back at the dial's rest", () => {
    // Not a copy built out of multiplications by one: a biome that
    // differs from its own row in the last bit of a float is a biome
  ...
  it("never stands sand steeper than its own angle of repose", () => {
    ...
    // steepest point in the biome is not the same point at two dial
  ...
  it("measures the SAND against repose, and not the rock it lies on", () => {
    ...
    // red for a biome that had done nothing wrong.
    ...
    // surface and this biome is refused for a wall it has not built.
  ...
  it("moves nothing in a biome with no sand", () => {
