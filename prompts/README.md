...
No prompts yet: the game has no LLM features, and the repo's agent guidance lives in [AGENTS.md](../AGENTS.md) and `.agent/skills/` instead. The first candidate for this directory is a stage-describer prompt (turning a generated stage's segment plan into pacenotes-style copy); when it lands it goes in `prompts/stage-pacenotes/1_0_0.md` following the format above.
