...
proportional to how far BEHIND the biome it has fallen, and the lag is a
...
biome's rise the road took stayed at 0.27. The cap was never binding,
...
- **Measure the ride CONDITIONED on the land being steep.** A desert
  ...
  samples where the bare land under the road runs over ~8%, then compare
  the road's rise to the land's over the same window.
- **`grade` and `lag` are ROAD-BUILDING numbers, not mountain ones.** A
  test that asserts `land.grade === 1` on every non-alpine biome as a
  proxy for "carries none of R47" breaks the first time a flat land
