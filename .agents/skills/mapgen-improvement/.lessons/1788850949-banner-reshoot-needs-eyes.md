...
`make biomes` after a generator change re-shot all three biome banners and
...
11 KB — despite both biomes being provably unchanged (digest parity clean,
`make routes` moving only the alpine's polylines).

The tell was the two banners that had no business changing at all. A biome
...
sizes of the biomes the change did not touch. A stale banner is a much
