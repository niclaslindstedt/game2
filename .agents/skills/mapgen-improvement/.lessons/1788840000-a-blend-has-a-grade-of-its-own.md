---
title: A blend has a grade of its own — bound a run-out at its reach, never ease it toward the biome
date: 2026-09-05
scope: engine/mapgen/terrain.ts
concepts: [terrain, r31, verge, embankment, spurs, measurement]
---

A fill's side falls at the verge grade and lands on the land by a
...
biome at the reach, so there is no seam either; the corridor's ease
...
height by the reach — read off the land under the centerline, cached
per sample. The same rule serves a branch's fill (`spurs.highest`), with the
biome under the branch read once per query.
