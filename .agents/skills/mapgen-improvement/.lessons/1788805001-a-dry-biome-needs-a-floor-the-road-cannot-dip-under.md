---
title: A biome with no water still has to keep its ground above LAKE_Y — and the ROAD's own roll dips under the ground, so the floor clears elevation.amplitude too
date: 2026-09-02
...
that if the rock still runs below the table — and a low biome's hollows do.

So a dry biome gets a FLOOR (`BiomeLand.floor`, a soft quadratic knee in
`geology.ts` so nothing creases), and the floor has to clear more than the
table: the road rides its own rolling profile on top of the biome
...
the table, and `buildRolling` scales the roll by the biome's relief.

The analyzer's bands are the biome's too (`ANALYSIS.ground.biome`):
