...
| The WHEEL that comes off: a cylinder with mass, grip and a gyroscope, met by the ground, the car and the solids                              | `pwa/src/game/loose-wheel.ts` (DOM-free; `tests/loose_wheel_test.ts`) off `car-damage.ts`; `make wheel`     |
