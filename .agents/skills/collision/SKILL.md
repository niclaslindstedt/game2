...
| What the WHEELS ride over instead of the body hitting: low solids and the anti-cut blocks, one `mount`                                       | `engine/game/collision.ts` (`clipSolids`, `clipKerbs`, `ridesOver`) + `TUNING.collision.rideOver` / `.kerb`                                          |
| The piece a felled solid leaves behind, flying                                                                                               | `pwa/src/game/breakage.ts`, over `tumble.ts`; retired via `world.fell`                                                                               |
| The WHEEL that comes off: a cylinder with mass, grip and a gyroscope, met by the ground, the car and the solids                              | `pwa/src/game/loose-wheel.ts` (DOM-free; `tests/loose_wheel_test.ts`) off `car-damage.ts`, switched by `LOOSE_WHEELS` in `settings.ts`; `make wheel` |
| The GROUND as a solid: a face too steep to climb, met at the bumper                                                                          | `engine/game/collision.ts` (`collideSlope`) + `ground.ts` (`hitFace`)                                                                                |
| Where the car STANDS: the seat, the edge and the kink that throw it, the bump the springs are handed                                         | `engine/game/ground.ts` (`readSeat`, `standOn`, `groundJolt`), `TUNING.air`                                                                          |
...
| What a damaged car DRIVES like: the whole ledger as multipliers (power, misfire, rack, pull, grip, brake, drag, gears)                       | `engine/game/damage.ts` + `TUNING.collision.chassis` — reads `car.damage`, never writes; spent in `car.ts`                                           |
...
| WHO WRITES a mesh's colours when two writers want to: the base (fold, lit, scuffed) and the coat (dirt), composed in that order              | `pwa/src/game/car-paint.ts` — neither writer touches the buffer directly                                                                             |
...
5. **LOOK.** `make wrecks` first — every accident the ledger can describe,
   staged on one body with no physics in the way, before and after — then
   build and drive a crash on purpose (playtest-style script:
