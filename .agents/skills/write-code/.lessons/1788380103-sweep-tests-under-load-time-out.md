---
title: Never run the sweep suites while `make sim` or `make analyze` runs in the background — their 20 s timeouts read as eight regressions
date: 2026-09-02
scope: tests/
...
20 s timeout, and a background `make analyze COUNT=24` or `make sim` on the
