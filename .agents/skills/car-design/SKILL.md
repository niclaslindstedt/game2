...
| `pwa/src/game/car-lamps.ts`         | The beams the PLAYER's lamps throw on the road (the lamps themselves are `car/lamps.ts`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `pwa/src/game/car/lamps.ts`         | The lamps' bowls — AND the light the car throws: one source per housing derived off its bowl (`headLampSources` / `tailLampSources`), hung by `environment.ts` and capped by `LAMP_BEAMS`. The BRAKE lights are the marker here plus the signal over it — `BRAKE_LIGHT` and the brake pool in `environment.ts`, `BRAKE_DAY`/`BRAKE_NIGHT` in `car-mesh.ts` (the bloom, which is the whole daylight read). `headLampSources` / `tailLampSources` derive one beam per housing off its own bowl, so restyling a face changes what the car lights the night with. `size`, `pairGap`/`pairSize` and `lampPods` are load-bearing for the LIGHTING row, not only for the look |
