...
| `pwa/src/game/car/interior.ts`      | What is BEHIND the glass: trim, seats, the crew, the cage, the wheel                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `pwa/src/game/car-lamps.ts`         | The beams the PLAYER's lamps throw on the road, and the BRAKE pool behind (`BRAKE_LIGHT`) — the lamps themselves are `car/lamps.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `pwa/src/game/car-glow.ts`          | The light those lamps put back on the car CARRYING them: a register of its own lamp anchors, grafted into the bodywork's shaders so the nose warms and the tail goes red after dark. Half under the paint and half over it, or a dark car never reads as lit. The cabin's furniture is exempt by material name (`CABIN_TRIM_MATERIAL`) — a lamp on the outside of a closed box does not light the room behind it                                                                                                                                                                |
| `pwa/src/game/car/greenhouse.ts`    | Windows cut out of a solid cabin; gutters; the two screens as glass (`screenPanes`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `pwa/src/game/car/wipers.ts`        | The screens' grime film and the blades that sweep it — arms, park and sweep angles, how fast the glass soils. The film is double-sided: the driver looks through it from inside                                                                                                                                                                                                                                                                                                                                                                                                 |
| `pwa/src/game/car/cockpit.ts`       | The first-person cabin the player's car alone is built with, and the `MirrorMount` the rear-view pass stands its lens on; the room it shares with the dials is `cockpit-room.ts`                                                                                                                                                                                                                                                                                                                                                                                                |
| `pwa/src/game/car/cockpit-dials.ts` | The binnacle and the tripmeter: dials with figures, the gear readout, the shift lamp, the tell-tales — over `segment-display.ts`'s seven-segment bars; the readings come off `car-instruments.ts`. `tests/cockpit_test.ts` holds it, and a 4x crop of a cockpit shot is how it is judged                                                                                                                                                                                                                                                                                        |
...
| `pwa/src/game/car-crew.ts`          | Also `MAP_READER` — one model in the driver's own `CrewColors`. The sixteen characters and their gear colours, plus who drives for which crew. **Pure data, no three.js**                                                                                                                                                                                                                                                                                                                                                                                                       |
| `pwa/src/game/car/fascia.ts`        | Nose and tail: grille, lamps, bumpers, air dam, plate, exhaust, the detachable bonnet and boot lid                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `pwa/src/game/car/lamps.ts`         | The lamps' bowls — AND the light the car throws: `headLampSources` / `tailLampSources` derive one source per housing off its own bowl, hung by `car-lamps.ts` and capped by `LAMP_BEAMS`, so restyling a face changes what the car lights the night with. `size`, `pairGap`/`pairSize` and `lampPods` are load-bearing for the LIGHTING row, not only for the look. The BRAKE lights are the marker here plus the signal over it — `BRAKE_LIGHT` and the brake pool in `car-lamps.ts`, `BRAKE_DAY`/`BRAKE_NIGHT` in `car-mesh.ts` (the bloom, which is the whole daylight read) |
...
| `pwa/src/game/car/screen-rain.ts`   | RAIN ON THE WINDSCREEN from the driver's own seat — a shader pane over the glass, drawn in a pass of its own after the frame it refracts. Where a wiper arm IS for anything else on the same glass is `WipeState` in `car/wipers.ts`; the pane's metric frame is `car/pane-frame.ts`                                                                                                                                                                                                                                                                                            |
| `pwa/src/game/car-shake.ts`         | How the body TREMBLES when the engine is being worked — DOM-free; read by `car-mesh.ts` (the shell) and `camera-eye.ts` (the seat)                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `pwa/src/game/car-shadow.ts`        | The shadow a car throws: the sun's shadow map — who casts, who receives, the frame — off `sunHardness` in `sky.ts` (the `atmosphere` skill decides how hard)                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
4. **Pick the winner, fold it into `car-styles.ts`, re-render.** Spec-only
   ...
   A car measured off a REAL car's photographs is the `car-creation`
   skill's pass: the ruled crop, the compression rule, and the overlay
   that proves the render against the picture.
