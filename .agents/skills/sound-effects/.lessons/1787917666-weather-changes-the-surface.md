...
date: 2026-08-28
scope: pwa/src/game/audio/road-voice.ts, pwa/src/game/audio/drive-bed.ts
...
the crossover, which is exactly where the two spectra overlap most, and it
is why the roar is two layers, one per colour.)
...
The rain itself is two layers with nothing to do with the car — they play
over a parked car and one in mid-air, past every early return in
`roadTargets`. The sheet high and broad, the patter on the panels narrow
and low, both lifting with speed, because a car at 140 km/h is driving
INTO the rain rather than being rained on.
