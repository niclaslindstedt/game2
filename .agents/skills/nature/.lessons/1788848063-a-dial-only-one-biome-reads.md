---
title: A dial only one biome reads goes in `landOf`, returns the ROW ITSELF at rest, and compares on the DIAL and not on the metres
date: 2026-09-08
...
- **The row is the biome at the dial's REST.** `landOf` hands the row object
  back by identity there, never a copy built out of multiplications by one —
  `1 - (1 - 0.3) * 1` is `0.30000000000000004`, and a biome that differs from
  its own row in the last float bit is a biome whose seeds differ from the
...
- **The cache key must grow with the dial** (`${biome}|${altitude}|${dunes}`) —
  ...
  missing the new dial serves one biome's row for another's.
...
Gate the row on a FLAG rather than on the biome's name (`BiomeLand.dunes !==
null`, `BiomeRules.blown`), so a fourth biome gets the row offered without
