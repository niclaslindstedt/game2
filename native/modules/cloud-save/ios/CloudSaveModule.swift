...
//
// NO GAME CENTER HERE. A driver's name on a board is the three initials they
// type in (`pwa/src/game/scores.ts`), not an account — so there is nothing to
// authenticate and nothing to sign into. The save follows the Apple ID the
// device is already signed into, which is all iCloud key-value storage needs.
//
