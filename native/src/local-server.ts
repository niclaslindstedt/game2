...
// and secure-context storage (localStorage, where the campaign, the score
// boards and the ghost live) behaves identically to the deployed site. The
// port is FIXED — the WebView origin is `http://<host>:<port>`, and browser
// storage is keyed to that origin, so a stable port is what keeps a player's
// progress across launches.
//
...
export type LocalServer = {
  /** The origin to point the WebView at, e.g. `http://localhost:9007`. */
  origin: string;
