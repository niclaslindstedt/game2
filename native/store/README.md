...
Everything App Store Connect, the Play Console and Steamworks need, generated
...
make store-metadata          # copy.mts + listing.mts → store.config.json, fastlane, Steam page
...
| `listing.mts`                        | The RULES: types, limits, categories, age rating, Steam tags | yes        |
...
| `../../tauri/store/steam-listing.md` | The Steam store page, to paste into Steamworks               | no (built) |
...
contact, the Steam tags, and `notYetShipped`. An age-rating answer is a claim
...
| `releaseNotes`           | ≤ 4000                                      |
| `review.notes`           | 2–4000                                      |
| `steam.shortDescription` | ≤ 300 (Valve's)                             |
...
- The Steam copy may not mention anything in `steam.notYetShipped`. Valve reviews
  the page and the build **together**; as the shell grows a feature, drop its row
  and the copy is free to say so.
...
smaller device in its family — plus **Steam** (1920×1080), which lands under
`tauri/store/screenshots/` instead, because the fastlane upload ships everything
...
`tauri/store/` — the store page copy (generated here), the screenshots at
Valve's raster, the capsule art, and the app and depot ids. See
[`../../tauri/store/README.md`](../../tauri/store/README.md).
...
- Steam's capsule art, which is designed rather than generated — a capsule is the
  game's name set in a picture, which is a different job from an icon.
